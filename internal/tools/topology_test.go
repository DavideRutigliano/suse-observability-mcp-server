...
func TestGetComponents(t *testing.T) {
	...
	t.Run("success with withNeighborsOf", func(t *testing.T) {
		...
		// Expected STQL query (Updated expectation to match ANY order of clauses since map iteration is random if implementation used map, but here slice is used so order is fixed by logic)
		// name IN ("db-master") OR withNeighborsOf(components = (name IN ("db-master")), levels = "2", direction = "down")
